// The original has been taken from: https://github.com/Thomas-Smyth/SquadJS
// Authors: Enrique Carpintero, Larinel, Thomas Smyth and Marcel Davis
...
class Rcon extends EventEmitter {
  ...
  onData(data) {
    ...
    for (const packet of packets) {
      ...
      Logger.verbose('RCON', 3, `Processing decoded packet: ${this.decodedPacketToString(decodedPacket)}`);
      ...
      switch (decodedPacket.type) {
        ...
        default:
          Logger.verbose(
            'RCON',
            1,
            `Unknown packet type ${decodedPacket.type} in: ${this.decodedPacketToString(decodedPacket)}`
          );
      }
    }
  }
  ...
  write(type, body) {
    return new Promise((resolve, reject) => {
      ...
      const encodedPacket = this.encodePacket(type, type !== SERVERDATA_AUTH ? MID_PACKET_ID : END_PACKET_ID, body);
      ...
      if (type === SERVERDATA_AUTH) {
        ...
        this.responseCallbackQueue.push((response) => {
          ...
          Logger.verbose('RCON', 2, `Returning complete response: ${response.replace(/\r\n|\r|\n/g, '\\n')}`);
      ...
      if (type !== SERVERDATA_AUTH) {
        Logger.verbose('RCON', 4, `Sending empty packet: ${this.bufToHexString(encodedEmptyPacket)}`);
